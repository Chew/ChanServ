module Op
  ...
  command(:op, min_args: 1, max_args: 1) do |event, mention|
    unless %w[Oper Owner Admin].include? role(event.user, event.server).to_s
    ...
    to_add = event.server.roles.find { |role| role.name == 'Ops' }
    user.add_role(to_add)

    event.channel.send_embed do |e|
      e.title = '**User Mode Changed Successfully**'

      e.description = "#{user.mention} has been opped by #{event.user.mention}."
      e.color = '00FF00'
    end
    ...
    message = Bot.channel(210_174_983_278_690_304).send_embed do |embed|
      ...
      embed.add_field(name: 'Mode', value: 'Op (+o)', inline: true)
