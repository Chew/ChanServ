module Owner
  ...
  command(:owner, min_args: 1, max_args: 1) do |event, mention|
    ...
    to_add = event.server.roles.find { |role| role.name == 'Owner' }
    user.add_role(to_add)

    event.channel.send_embed do |e|
      e.title = '**User Mode Changed Successfully**'

      e.description = "#{user.mention} has been promoted to owner by #{event.user.mention}."
      e.color = '00FF00'
    end
    ...
    message = Bot.channel(210_174_983_278_690_304).send_embed do |embed|
      ...
      embed.add_field(name: 'Mode', value: 'Owner (+q)', inline: true)
